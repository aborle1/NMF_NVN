#Use CP-SAT for experiment 3 (this assumes you already have dirac-3 results)
import numpy as np
...
device = 'dirac3'
type = 'potts'
thread = 'sthread' #sthread is for serial processing, mthread is for parallel processing
objective = 'l1' #l1 : sum of abs diff, l2: sum of squared diff
value_range = (0,7) # range of integers
...
for n in [1,2,3,4]:
    ...
    for i in range(0,100):
        ...
        filepath = "./test_matrices/dirac3_exp3/size_" + str(n) + "x" + str(m) + "/" + filename 
        f = np.load(filepath)
        V = f['V']
        f.close()
        ...
        res_filepath = "./test_matrices/dirac3_exp3/size_" + str(n) + "x" + str(m) + "/results/" + res_filename 
        ...
        for j in range(num_samples): #num_samples = 10 as dirac3 was run 10 times as welll
            #serial processing
            if thread == 'sthread':
                W_cpsat, H_cpsat, info = integer_nmf_ortools(V, n_components=k,max_time=dirac3_time,init=False,seed=seed,threads=1,objective=objective,value_range=value_range)
            #parallel processing
            else:
                #if thread parameter is not given, then max number of processors (offered by a computer) are chosen  ; in our case it is 12
                W_cpsat, H_cpsat, info = integer_nmf_ortools(V, n_components=k,max_time=dirac3_time,init=False,seed=seed,objective=objective,value_range=value_range)
            ...
            num_var = len(info['n_variables']) #Get the number of variables involved in the CP_SAT process
        ...
        print("Number of variables used CPSAT:",num_var)
        ...
        result_dict['n_variables'] = copy.deepcopy(num_var)



        res_filename2 = 'res_' + 'mat_'+str(n)+'x'+str(m)+"_"+str(i)+"_cpsat_"+ str(thread) +"_" + str(objective)  +"_" + str(value_range[1]) +".npz"
        res_filepath2 = "./test_matrices/dirac3_exp3/size_" + str(n) + "x" + str(m) + "/results/" + res_filename2 
