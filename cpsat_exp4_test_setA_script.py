#Use CP-SAT for dirac3 experiment 4 (this assumes you already have the dirac-3 results)
#For test set A
import numpy as np
...
thread = 'sthread' #sthread for serial processing, mthread for parallel processing
objective = 'l2'
...
for i in range(83,100):
    ...
    for j in range(num_samples): #num_samples = 10 as dirac3 was run 10 times as welll
        if thread == 'sthread':
            W_cpsat, H_cpsat, _ = integer_nmf_ortools(V, n_components=k,max_time=dirac3_time,init=False,seed=seed,threads=1,objective=objective,value_range=value_range)
        else:
            #If no threads are specified, then its taken to mean that CP-SAT should use ALL available processors (12 in our work)
            W_cpsat, H_cpsat, _ = integer_nmf_ortools(V, n_components=k,max_time=dirac3_time,init=False,seed=seed,objective=objective,value_range=value_range)
    ...
    res_filepath2 = "./test_matrices/dirac3_exp4/size_" + str(n) + "x" + str(m) + "/results/" + res_filename2 
